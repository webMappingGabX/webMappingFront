...
const MessageBox = ( { content, title = "Alerte" } ) => {
    const { setIsPopupVisible, isErrorMessage } = useAppMainContext();
    ...
    return (
        <>
            ...
            <div className="fixed z-[3001] shadow-md flex flex-col rounded-md p-4 bg-white top-1/2 left-1/2 -translate-1/2 cursor-default">
                <h1 className={`mb-8 text-xl text-center ${ isErrorMessage ? `text-red-500` : `text-green-500` } md:text-2xl`}>{ isErrorMessage ? `Echec de l'opération` : `Opération réussie` }</h1>
